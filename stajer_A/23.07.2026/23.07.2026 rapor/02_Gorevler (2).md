...
3 led vardı. for döngülerini; Led_Init, Led_SelfTest ve Led_ChaseStep fonksiyonlarında kullandım.

2. `if` ile butonu veya `fast_mode` / `reverse` nasıl kontrol ettin?  
Buton dosyasından gelen olayları (event) ana döngüde if ve else if yapılarıyla kontrol ettim. Eğer gelen olay EVENT_SHORT_PRESS (kısa basış) ise reverse = !reverse; diyerek akış yönünü değiştirdim; eğer olay EVENT_LONG_PRESS (uzun basış) ise fast_mode = !fast_mode; diyerek hız modunu değiştirdim.

3. `const` bir sabit ile normal değişken (`led_on` gibi) farkını **kendi kodundan** örnekle.  
const ile tanımladığım CHASE_SLOW_MS = 250; değişkeni sabit bir referans süresidir ve program çalışırken bellekte asla değiştirilemez. Ancak bool reverse = false; gibi normal bir değişken bellekte tutulur ve butona basılma koşullarına bağlı olarak program akışı sırasında sürekli olarak değer değiştirebilir.

4. (Zor yaptıysan) Kesme mi debounce mu; neden onu seçtin?
debounce kullandım. Butondaki fiziksel yaylanmaları (bouncing) 20 ms'lik bir filtre ile yok edip, aynı buton üzerinden kronometre mantığıyla hem "kısa" hem de "uzun" basış olmak üzere iki farklı kararlı komut üretemk için.
